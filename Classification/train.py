...
from timm.data import Dataset, resolve_data_config, Mixup, FastCollateMixup, AugMixDataset  # , create_loader,timm-0.3.2
...
from dhs_vig_pytorch.data.myloader import create_loader
import dhs_vig_pytorch.dhs_vig

# TRUNCATE for bad images in ImageNet dataset
...
def main():
    ...
    if args.evaluate:
        if args.model_ema:
            eval_metrics = validate(model_ema.ema, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
            print(eval_metrics)
            torch.save(model_ema.ema.state_dict(), "ema_model.pth")
