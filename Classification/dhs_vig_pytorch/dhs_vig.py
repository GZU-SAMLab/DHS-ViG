...
class dhs_vig(torch.nn.Module):
    def __init__(self, opt):
        ...
        mode = opt.mode
        self.MSA_input_layer = opt.MSA_input_layer
        self.MSA_output_layer = opt.MSA_output_layer
        self.MSA_type = opt.MSA_type
        ...
        if self.MSA_type == 'Stem':
            self.feature_extractor = Stem(in_dim=channels[1], out_dim=channels[-1], act=act)
        elif self.MSA_type == "FFN":
            self.feature_extractor = FFN(in_features=channels[2], hidden_features=channels[2] * 4,
                                         out_features=channels[2],
                                         act=act, drop_path=drop_path)
    ...
    def forward(self, inputs):
        ...
        for i in range(len(self.backbone)):
            ...
            if i == self.MSA_input_layer:
                hidden_map = self.feature_extractor(x)

            if i == self.MSA_output_layer:
                x = x + hidden_map
...
@register_model
def dhs_vig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, mode='hds', **kwargs):
            ...
            self.MSA_input_layer = 4    # input layer for MSA
            self.MSA_output_layer = 14  # output layer for MSA
            self.MSA_type = 'Stem'  # type of MSA, Stem or FFN

    opt = OptInit(**kwargs)
    model = dhs_vig(opt)
    model.default_cfg = default_cfgs['dhs_vig_224_gelu']
    return model
...
@register_model
def dhs_vig_m_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, mode='hds', **kwargs):
            self.k = 9  # neighbor num (default:9)
            self.conv = 'mr'  # graph conv layer {edge, mr}
            ...
            self.use_stochastic = False  # stochastic for gcn, True or False
            self.drop_path = drop_path_rate
            self.blocks = [2, 3, 15, 2]  # number of basic blocks in the backbone
            self.channels = [96, 192, 384, 768]  # number of channels of deep features
            self.n_classes = num_classes  # Dimension of out_channels
            self.emb_dims = 1024  # Dimension of embeddings
            self.mode = mode  # Diff type GvT blocks
            self.MSA_input_layer = 6  # input layer for MSA
            self.MSA_output_layer = 21  # output layer for MSA
            self.MSA_type = 'FFN'  # type of MSA, Stem or FFN
...
@register_model
def dhs_vig_b_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, mode='hds', **kwargs):
            self.k = 9  # neighbor num (default:9)
            self.conv = 'mr'  # graph conv layer {edge, mr}
            ...
            self.use_stochastic = False  # stochastic for gcn, True or False
            self.drop_path = drop_path_rate
            self.blocks = [3, 4, 17, 2]  # number of basic blocks in the backbone
            self.channels = [128, 256, 512, 1024]  # number of channels of deep features
            self.n_classes = num_classes  # Dimension of out_channels
            self.emb_dims = 1024  # Dimension of embeddings
            self.mode = mode  # Diff type GvT blocks
            self.MSA_input_layer = 8  # input layer for MSA
            self.MSA_output_layer = 25  # output layer for MSA
            self.MSA_type = 'FFN'  # type of MSA, Stem or FFN

    opt = OptInit(**kwargs)
    model = dhs_vig(opt)
    model.default_cfg = default_cfgs['dhs_vig_b_224_gelu']
    return model